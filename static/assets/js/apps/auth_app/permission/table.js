    ...
    var userPermissionTable;
        ...
        $(document).ready(function () {
            if ($.fn.dataTable.isDataTable('#userPermissionTable')) {
                $('#userPermissionTable').DataTable().destroy();
            }

            userPermissionTable = $('#userPermissionTable').DataTable({
            pageLength: 50,
            ...
            {name: "module", data: "module"},
           {
            ...
            //                 </span>
            //             </button>
            //             <button class="btn btn-primary btn-icon btn-sm rounded-pill ms-2 edit-user-group" role="button">
            //                 <span class="btn-inner">
            //                     <!-- Placeholder for the second SVG icon -->
            //                     <svg class="icon-32" width="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    $('#userPermissionTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');

    // Initially hide the delete button
    $('#delete-permission').hide();


    /* Handle the select/deselect all functionality */
    $('#select-all').on('click', function() {
        var rows = userPermissionTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#userPermissionTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    var selectedIds = $('#userPermissionTable input[type="checkbox"]:checked').map(function() {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        userPermissionTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#userPermissionTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
...
function toggleButtons() {
    if ($('#userPermissionTable input[type="checkbox"]:checked').length > 0) {
        // If any checkboxes are selected, show the delete button and hide the other buttons
        $('#delete-permission').show();
        $('#add-user-group').hide();
        $('#add-permission').hide();
        $('#add-user-role').hide();
    } else {
