...
var equipmentTable;
    ...
    $(document).ready(function () {
        ...
        /* Set up the data source */
         ajax: {
            url: "/ajax_datatable/equipment/",
            data: function(d) {
                d.status = $('#equipment-status-filter').val();
        ...
        columns: [
            ...
            {name: "quantity_on_hand", data: "quantity_on_hand"},
            {name: "status", data: "status"},
            {name: "maintenance_schedule", data: "maintenance_schedule"},
...

            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center list-user-action">
                       ...
                       <a class="btn btn-sm btn-icon btn-warning edit-equipment" data-bs-toggle="tooltip" data-bs-placement="top" data-original-title="Edit" href="#" aria-label="Edit" data-bs-original-title="Edit">
                          <span class="btn-inner">
    ...
    $('#equipment-status-filter, #category-filter, #subcategory-filter').change(function() {
        equipmentTable.ajax.reload();
    });

    // Hide the dosage form filter initially
    // Initialize the select2 dropdowns
    $('#item-status-filter, #category-filter, #subcategory-filter').select2();



    /* Add a checkbox in the header for selecting/deselecting all rows */
    $('#equipmentTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');
    ...
    $('#select-all').on('click', function() {
        var rows = equipmentTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#equipmentTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    var selectedIds = $('#equipmentTable input[type="checkbox"]:checked').map(function() {
    ...
    if (selectedIds.length > 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/equipment/delete/',  // URL for your delete view
                    type: 'POST',
                    headers: {
                ...
                $('#equipmentTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
...
function toggleButtons() {
    if ($('#equipmentTable input[type="checkbox"]:checked').length > 0) {
