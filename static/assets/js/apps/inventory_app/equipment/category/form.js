  ...
  $(document).ready(function () {
      const element = document.getElementById('equipmentCategoryModal');
      const form = element.querySelector('#equipmentCategoryForm');
      ...
      $(form).on('submit', function(e) {
          ...
          validator.validate().then(function(status) {
              if (status === 'Valid') {
                  ...
                  $.ajax({
                    url: $('#equipmentCategoryForm').attr('action'),  // the url where we want to POST
                      headers: { "X-CSRFToken": getCookie("csrftoken") },
                      type: 'POST',  // define the type of HTTP verb we want to use (POST for our form)
                      data: new FormData($('#equipmentCategoryForm')[0]),   // our data object
                      ...
                      success: function(data) {
                          ...
                          if (data.success) {
                              ...
                              // Refresh the table
                              equipmentCategoryTable.ajax.reload()
                              Swal.fire({
                                  ...
                                  if (result.isConfirmed) {
                                      const formElement = document.querySelector('#equipmentCategoryForm'); // Replace with your form's ID

                                      formElement.reset(); // Reset form
                                      // Close the modal when the user clicks "Ok, got it!"
                                      $('#equipmentCategoryModal').modal('hide');
