...
var equipmentCategoryTable;

    $(document).ready(function () {
        /* Here begins the DataTable configuration. */
        if ($.fn.dataTable.isDataTable('#equipmentCategoryTable')) {
        $('#equipmentCategoryTable').DataTable().destroy();
    }

        equipmentCategoryTable = $('#equipmentCategoryTable').DataTable({
        ...
        ajax: {
            url: "/ajax_datatable/equipment_category/"
        },
            ...
            {name: "description", data: "description"},
            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center list-user-action">
                       ...
                       <a class="btn btn-sm btn-icon btn-warning edit-equipment-category" data-bs-toggle="tooltip" data-bs-placement="top" data-original-title="Edit" href="#" aria-label="Edit" data-bs-original-title="Edit">
                          <span class="btn-inner">
    ...
    $('#equipmentCategoryTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');

    // Initially hide the delete button
    $('#delete-equipment-category').hide();


    /* Handle the select/deselect all functionality */
    $('#select-all').on('click', function() {
        var rows = equipmentCategoryTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#equipmentCategoryTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    $('#delete-equipment-category').on('click', function() {
    console.log('Delete button clicked');  // Add this line

    // Get the IDs of the selected user groups
    var selectedIds = $('#equipmentCategoryTable input[type="checkbox"]:checked').map(function() {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        equipmentCategoryTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#equipmentCategoryTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
