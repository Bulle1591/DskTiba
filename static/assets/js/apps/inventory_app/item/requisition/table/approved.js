...
var approvedRequisitionTable;

$(document).ready(function () {
    /* Here begins the DataTable configuration. */
    if ($.fn.dataTable.isDataTable('#approvedRequisitionTable')) {
        $('#approvedRequisitionTable').DataTable().destroy();
    }

    approvedRequisitionTable = $('#approvedRequisitionTable').DataTable({
        ...
        ajax: {
            url: "/ajax_datatable/requisitions/approved/", // Update the URL to point to the endpoint for draft requisitions
            data: function(d) {
        ...
        columns: [
                ...
                },
                {
                    ...
                    render: function ( data, type, row, meta ) {
                        if(type === 'display'){
                            return '<a href="#" data-id="' + row.id + '" onclick="showApprovedRequisitionDetails(this)">' + data + '</a>';
                        }
                        return data;
                    }
                },
    ...
    $('#approvedRequisitionTable tbody').on('click', 'td.details-control', function (e) {
    e.stopPropagation(); // Prevent the click event from propagating up to the document

    var tr = $(this).closest('tr');
    var row = approvedRequisitionTable.row(tr);
    ...
    for (var i = 0; i < row.data().items.length; i++) {
        items += '<a href="/item-details/' + row.data().items[i].id + '" class="item-link" data-item-id="' + row.data().items[i].id + '">' + row.data().items[i].item_name + ' | Approved Quantity: ' + row.data().items[i].quantity + '</a><br>';
    }
