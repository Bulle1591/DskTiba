...
var itemSubcategoryTable;

    $(document).ready(function () {
        /* Here begins the DataTable configuration. */
        if ($.fn.dataTable.isDataTable('#itemSubcategoryTable')) {
        $('#itemSubcategoryTable').DataTable().destroy();
    }

        itemSubcategoryTable = $('#itemSubcategoryTable').DataTable({
        ...
        ajax: {
            url: "/ajax_datatable/item_subcategory/"
        },
            ...
            {name: "description", data: "description"},
            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center list-user-action">
                       ...
                       <a class="btn btn-sm btn-icon btn-warning edit-item-subcategory" data-bs-toggle="tooltip" data-bs-placement="top" data-original-title="Edit" href="#" aria-label="Edit" data-bs-original-title="Edit">
                          <span class="btn-inner">
    ...
    $('#itemSubcategoryTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');

    // Initially hide the delete button
    $('#delete-item-subcategory').hide();


    /* Handle the select/deselect all functionality */
    $('#select-all').on('click', function() {
        var rows = itemSubcategoryTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#itemSubcategoryTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    $('#delete-item-subcategory').on('click', function() {
    console.log('Delete button clicked');  // Add this line

    // Get the IDs of the selected user groups
    var selectedIds = $('#itemSubcategoryTable input[type="checkbox"]:checked').map(function() {
    ...
    if (selectedIds.length > 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/item_subcategory/delete/',  // URL for your delete view
                    type: 'POST',
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        itemSubcategoryTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#itemSubcategoryTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
