...
$(document).ready(function () {
    ...
    if ($.fn.dataTable.isDataTable('#allOrderTable')) {
        $('#allOrderTable').DataTable().destroy();
    }
    ...
    allOrderTable = $('#allOrderTable').DataTable({
        ...
        ajax: {
            url: "/orders/table/", // Update the URL to point to the endpoint for draft requisitions
            data: function(d) {
        ...
        columns: [
            ...
            { name: "ordered_by", data: "ordered_by" },
            { name: "ordered_at", data: "ordered_at" },
            {
    ...
    $('#allOrderTable tbody').on('click', 'td.details-control', function (e) {
    e.stopPropagation(); // Prevent the click event from propagating up to the document

    var tr = $(this).closest('tr');
    var row = allOrderTable.row(tr);
    ...
    for (var i = 0; i < row.data().items.length; i++) {
        items += '<a href="/item-details/' + row.data().items[i].id + '" class="item-link" data-item-id="' + row.data().items[i].id + '">' + row.data().items[i].item_name + ' | Approved Quantity: ' + row.data().items[i].approved_quantity + '</a><br>';
    }
