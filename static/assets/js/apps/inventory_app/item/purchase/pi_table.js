...
var purchaseItemTable;
    ...
    $(document).ready(function () {
        ...
        if ($.fn.dataTable.isDataTable('#purchaseItemTable')) {
        $('#purchaseItemTable').DataTable().destroy();
    }

        purchaseItemTable = $('#purchaseItemTable').DataTable({
        ...
        /* Set up the data source */
         ajax: {
            url: "/ajax_datatable/purchased/items/",
            data: function(d) {
        ...
        columns: [
            ...
            {name: "item_name", data: "item_name"},
            {name: "purchase_order", data: "purchase_order"},
...

            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center list-user-action">
                       ...
                       <a class="btn btn-sm btn-icon btn-warning edit-item" data-bs-toggle="tooltip" data-bs-placement="top" data-original-title="Edit" href="#" aria-label="Edit" data-bs-original-title="Edit">
                          <span class="btn-inner">
    ...
    $('#purchaseItemTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');
    ...
    $('#select-all').on('click', function() {
        var rows = purchaseItemTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#purchaseItemTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    var selectedIds = $('#purchaseItemTable input[type="checkbox"]:checked').map(function() {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        purchaseItemTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#purchaseItemTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
...
function toggleButtons() {
    if ($('#purchaseItemTable input[type="checkbox"]:checked').length > 0) {
