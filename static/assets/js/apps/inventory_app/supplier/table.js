...
var supplierTable;
    ...
    $(document).ready(function () {
        ...
        columns: [
            ...
            {name: "contact_number", data: "contact_number"},
...

            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center list-user-action">
                       ...
                       <a class="btn btn-sm btn-icon btn-warning edit-supplier" data-bs-toggle="tooltip" data-bs-placement="top" data-original-title="Edit" href="#" aria-label="Edit" data-bs-original-title="Edit">
                          <span class="btn-inner">
    ...
    $('#supplierTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');

    // Initially hide the delete button
    $('#delete-supplier').hide();


    /* Handle the select/deselect all functionality */
    $('#select-all').on('click', function() {
        var rows = supplierTable.rows({ 'search': 'applied' }).nodes();
    ...
    var selectedIds = $('#supplierTable input[type="checkbox"]:checked').map(function() {
    ...
    if (selectedIds.length > 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/api/supplier/delete/',  // URL for your delete view
                    type: 'POST',
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        supplierTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#supplierTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
...
function toggleButtons() {
    if ($('#supplierTable input[type="checkbox"]:checked').length > 0) {
