...
var designationTable;
    ...
    $(document).ready(function () {
        ...
        ajax: {
            url: "/ajax_datatable/designation/"
        },
...

            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center">
                        ...
                        <button class="btn btn-primary btn-icon btn-sm rounded-pill ms-2 edit-designation" role="button">
                            <span class="btn-inner">
    ...
    $('#designationTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');

    // Initially hide the delete button
    $('#delete-designation').hide();


    /* Handle the select/deselect all functionality */
    $('#select-all').on('click', function() {
        var rows = designationTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#designationTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    var selectedIds = $('#designationTable input[type="checkbox"]:checked').map(function() {
    ...
    if (selectedIds.length > 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/api/designation/delete/',  // URL for your delete view
                    type: 'POST',
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        designationTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#designationTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
