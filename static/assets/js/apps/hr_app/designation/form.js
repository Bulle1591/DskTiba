  ...
  $(document).ready(function () {
      const element = document.getElementById('designationModal');
      const form = element.querySelector('#designationForm');
      ...
      form,
          {
              fields: {
                  title: {
                      validators: {
                          notEmpty: {
                              message: "Designation title can not be empty!"
                          }
                      }
                  },
                  description: {
      ...
      $(form).on('submit', function(e) {
          ...
          validator.validate().then(function(status) {
              if (status === 'Valid') {
                  ...
                  $.ajax({
                    url: $('#designationForm').attr('action'),  // the url where we want to POST
                      headers: { "X-CSRFToken": getCookie("csrftoken") },
                      type: 'POST',  // define the type of HTTP verb we want to use (POST for our form)
                      data: new FormData($('#designationForm')[0]),   // our data object
                          ...
                          if (data.success) {
                              ...
                              Swal.fire({
                                  ...
                                  if (result.isConfirmed) {
                                      const formElement = document.querySelector('#designationForm'); // Replace with your form's ID
