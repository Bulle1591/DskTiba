  ...
  $(document).ready(function () {
      const element = document.getElementById('subDepartmentModal');
      const form = element.querySelector('#subDepartmentForm');
      ...
      form,
          {
              fields: {
                  name: {
                      validators: {
                          notEmpty: {
                              message: "Sub Department can not be empty!"
                          }
                      }
                  },
                  department: {
                      validators: {
                          notEmpty: {
                              message: "Main Department can not be empty!"
                          }
                      }
                  }
//                  department_types: {
//                      validators: {
//                          notEmpty: {
//                              message: "Type of department can not be empty!"
//                          }
//                      }
//                  }
              },
      ...
      $(form).on('submit', function(e) {
          ...
          validator.validate().then(function(status) {
              if (status === 'Valid') {
                  ...
                  $.ajax({
                    url: $('#subDepartmentForm').attr('action'),  // the url where we want to POST
                      headers: { "X-CSRFToken": getCookie("csrftoken") },
                      type: 'POST',  // define the type of HTTP verb we want to use (POST for our form)
                      data: new FormData($('#subDepartmentForm')[0]),   // our data object
                      ...
                      success: function(data) {
                          ...
                          if (data.success) {
                              ...
                              // Refresh the table
                              subDepartmentTable.ajax.reload()
                              Swal.fire({
