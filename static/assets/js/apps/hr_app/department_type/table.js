...
var departmentTypeTable;
    ...
    $(document).ready(function () {
        ...
        if ($.fn.dataTable.isDataTable('#departmentTypeTable')) {
        $('#departmentTypeTable').DataTable().destroy();
    }
        ...
        ajax: {
            url: "/ajax_datatable/department_type/"
        },
...

            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center">
                        ...
                        <button class="btn btn-primary btn-icon btn-sm rounded-pill ms-2 edit-department-type" role="button">
                            <span class="btn-inner">
    ...
    $('#departmentTypeTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');

    // Initially hide the delete button
    $('#delete-department-type').hide();


    /* Handle the select/deselect all functionality */
    $('#select-all').on('click', function() {
        var rows = departmentTypeTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#departmentTypeTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    $('#delete-department-type').on('click', function() {
    console.log('Delete button clicked');  // Add this line

    // Get the IDs of the selected user groups
    var selectedIds = $('#departmentTypeTable input[type="checkbox"]:checked').map(function() {
    ...
    if (selectedIds.length > 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/api/department_type/delete/',  // URL for your delete view
                    type: 'POST',
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        departmentTypeTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#departmentTypeTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
