  ...
  $(document).ready(function () {
      const element = document.getElementById('jobTypeModal');
      const form = element.querySelector('#jobTypeForm');
      ...
      form,
          {
              fields: {
                  title: {
                      validators: {
                          notEmpty: {
                              message: "Employment type can not be empty!"
                          }
                      }
                  },
                  description: {
      ...
      $(form).on('submit', function(e) {
          ...
          validator.validate().then(function(status) {
              if (status === 'Valid') {
                  ...
                  $.ajax({
                    url: $('#jobTypeForm').attr('action'),  // the url where we want to POST
                      headers: { "X-CSRFToken": getCookie("csrftoken") },
                      type: 'POST',  // define the type of HTTP verb we want to use (POST for our form)
                      data: new FormData($('#jobTypeForm')[0]),   // our data object
                          ...
                          if (data.success) {
                              ...
                              Swal.fire({
                                  ...
                                  if (result.isConfirmed) {
                                      const formElement = document.querySelector('#jobTypeForm'); // Replace with your form's ID
