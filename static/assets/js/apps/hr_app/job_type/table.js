...
var jobTypeTable;
...

            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center">
                        ...
                        <button class="btn btn-primary btn-icon btn-sm rounded-pill ms-2 edit-job-type" role="button">
                            <span class="btn-inner">
    ...
    $('#jobTypeTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');

    // Initially hide the delete button
    $('#delete-job-type').hide();


    /* Handle the select/deselect all functionality */
    $('#select-all').on('click', function() {
        var rows = jobTypeTable.rows({ 'search': 'applied' }).nodes();
    ...
    $('#jobTypeTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    var selectedIds = $('#jobTypeTable input[type="checkbox"]:checked').map(function() {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        jobTypeTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#jobTypeTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
