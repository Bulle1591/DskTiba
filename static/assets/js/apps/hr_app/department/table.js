...
var departmentTable;
    ...
    $(document).ready(function () {
        ...
        columns: [
            ...
            {name: "name", data: "name"},
            {name: "code", data: "code"},
            //{name: "consultation", data: "consultation"},
            //{name: "discharge_approval", data: "discharge_approval"},
...

            {
                ...
                defaultContent: `
                    <div class="d-flex justify-content-center">
                        ...
                        <button class="btn btn-primary btn-icon btn-sm rounded-pill ms-2 edit-department" role="button">
                            <span class="btn-inner">
    ...
    $('#departmentTable thead th:eq(0)').html('<input type="checkbox" id="select-all">');
    ...
    $('#departmentTable tbody').on('change', 'input[type="checkbox"]', function() {
        if (!this.checked) {
    ...
    var selectedIds = $('#departmentTable input[type="checkbox"]:checked').map(function() {
    ...
    if (selectedIds.length > 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/api/department/delete/',  // URL for your delete view
                    type: 'POST',
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        departmentTable.ajax.reload(function() {
                            // Show or hide buttons based on whether any checkboxes are selected
                            toggleButtons();
                        }, false);
                ...
                $('#departmentTable input[type="checkbox"]').prop('checked', false);
                // Show or hide buttons based on whether any checkboxes are selected
                toggleButtons();
            }
        });
...
function toggleButtons() {
    if ($('#departmentTable input[type="checkbox"]:checked').length > 0) {
